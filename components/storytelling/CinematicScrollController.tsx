  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      panels.forEach((panel, index) => {
        ...
        scrubTimeline.to(
          panel,
          { opacity: 0, filter: 'blur(12px)', duration: 0.8, ease: 'power2.in' },
          0.6
        );
        ...
        ScrollTrigger.create({
          ...
          onEnter: () => {
            ...
            if (panels[index - 1]) {
              gsap.to(panels[index - 1], { opacity: 0, pointerEvents: 'none', duration: 0.6, ease: 'power2.in' });
            }
          },
          onEnterBack: () => {
            gsap.to(panel, { opacity: 1, pointerEvents: 'auto', duration: 0.7, ease: 'power3.out' });
            if (panels[index + 1]) {
              gsap.to(panels[index + 1], { opacity: 0, pointerEvents: 'none', duration: 0.5, ease: 'power2.in' });
            }
          },
          onLeave: () => {
